...
function onMouseClick(event) {
	...
	// Detect intersections with all objects in the scene
	let intersections = raycaster.intersectObjects([threejs_objects['scene']]);

	if (intersections.length > 0) {
		intersection = intersections[0];
		console.log('Intersection:', intersection);
		// 返回物体和射线的焦点
		// Handle the selected point, e.g., change color
		if (intersection.object.geometry.attributes.color) {
			const color = intersection.object.geometry.attributes.color;
			color.setXYZ(intersection.index, Math.random(), Math.random(), Math.random()); // Randomly change color
